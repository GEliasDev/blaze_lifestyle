...
// to a client's Exercise data, not read-only) — a coach browsing a real
// client can still reach Tags to see that client's set, though writes there
// are read-only for anyone but the tags' own owner (see ExerciseTagsScreen).
export function ExerciseBottomNav({ linkBase }) {
  ...
  const items = [
    ...
    { to: "tags", icon: Tag, key: "exercise.manageTags" },
